...
for key in GET_ON_LOAD_DATA_URL UPDATE_WORD_URL DELETE_WORD_URL ADD_IMPROV_WORD_URL UPDATE_SENTENCE_URL CLOUDFLARE_ASSETS_URL; do
  if [[ -z "${!key:-}" ]]; then
...
enum GeneratedEnv {
    ...
    static let addImprovWordURL = URL(string: "${ADD_IMPROV_WORD_URL}")!
    static let updateSentenceURL = URL(string: "${UPDATE_SENTENCE_URL}")!
